...
```sh
pi install git:github.com/liu-zhengdong/pi-stall-watchdog@v0.1.3
```
...
1. 把原会话当前分支整理成对话正文，写到 `~/.pi/agent/stall-watchdog/recovered/<原会话文件名>.md`。收用户消息、助手文字、工具调用（名称和参数开头）、压缩与分支摘要、显示出来的扩展消息；不收推理（thinking）和工具输出。最后一条用户消息放在最前面；
2. 新开会话，用 `parentSession` 链接到原会话（原会话文件原样保留）；
3. 写入一条接续说明（custom message，界面上可见，也进入模型上下文），指向这份正文，并提醒原会话文件含推理、不要打印进上下文；
4. 发送「继续上次的任务」。

不让新会话直接读原会话文件，是因为模型会自己解析 JSONL，把里面的推理打印进上下文。2026-09-23 连续三次拦截的类别都是 `reasoning_extraction`，其中两次紧跟在打印旧推理之后（只是相关，拦截规则不公开）。工具输出也不收，因为里面可能就是之前打印过的推理。正文写不出来时（如磁盘错误），说明里只给原会话路径，恢复照常进行。
